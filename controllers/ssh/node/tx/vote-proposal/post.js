...
const voteProposalCommand = require('../../../../../commands/node/tx/vote-proposal/default');
...
module.exports = (req, res) => {
  ...
  if (!req.body.proposal_id || typeof req.body.proposal_id != 'string' || !req.body.proposal_id.trim().length || req.body.proposal_id.length > DEFAULT_MAX_TEXT_FIELD_LENGTH)
    return res.json({ err: 'bad_request' });
  ...
  if (req.body.non_generic_tx_commands && Array.isArray(req.body.non_generic_tx_commands) && req.body.non_generic_tx_commands.includes('vote_proposal')) {
    if (req.body.chain_registry_identifier == 'celestiatestnet3') {
      command = voteProposalCommand_celestiatestnet3({
        fees: req.body.fees,
