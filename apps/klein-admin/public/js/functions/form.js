...
window.addEventListener('load', () => {
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.classList.contains('general-select-each-input-choice')) {
      const realValueInput = event.target.parentNode.parentNode.querySelector('.general-select-input-real-value');
      if (realValueInput.type == 'checkbox') {
        realValueInput.checked = Number(event.target.id.replace('select-input-', ''));
      } else {
        realValueInput.value = Number(event.target.id.replace('select-input-', ''));
      }
